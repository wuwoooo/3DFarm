              ...
              <view class="group2 flex-row">
                ...
                <view class="mod2 flex-col justify-between">
                  <view class="main1 flex-row justify-between">
                    <image class="pic4" src="@/static/dou.png" />
            ...
            <block v-for="(item,index) in showData" :key="index">
                <view class="box3 flex-row">
                  ...
                  <view class="bd1 flex-col justify-between">
                    <text class="word6">{{item.createTime}}&nbsp;豆{{item.flag==1?'收入':'支出'}}</text>
                    <text class="txt2">
                      {{item.note}}&nbsp;&nbsp;余额：{{utils.retain(Number(item.afterAmount),2)}}
                    </text>
                  </view>
                  <text class="word10" v-if="item.flag==1">+{{utils.retain(Number(item.changeAmount),2)}}</text>
                  <text class="word7" v-else>-{{utils.retain(Number(item.changeAmount),2)}}</text>
                </view>
...
export default {
  ...
  onLoad() {
      ...
      let requestdata = {lastId:-1,size:100,uuid:this.lib.User.uuid(),token:this.lib.User.token(),uid:this.lib.User.uid(),diamondCurrency:174}
      let method = 'GET'
      uni.showLoading({
          title: '加载中'
      })
  ...
  onNavigationBarButtonTap(e) {
      this.global.gotoWebView('https://img.tokenai.net/introduction_zy/mbeans_des.html')
  },
};
</script>
<style>
