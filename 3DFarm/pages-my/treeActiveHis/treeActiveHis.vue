          ...
          <view class="layer2 flex-col">
            <view class="box1 flex-col">
                <view class="flex-row" style="margin-top: 30rpx;">
                  <view class="right1 flex-row justify-between" style="margin-left: 100rpx;">
                    <image class="right3" src="@/pages-my/static/my3.png" />
                    <view class="right4 flex-col">
                      <view class="right5 flex-col justify-between">
                        <text class="right6">树苗</text>
                        <text class="right7">{{total}}</text>
                      </view>
                    </view>
                  </view>
                  <view style="border-left: 3rpx solid #b08d77;margin-top: 20rpx;height: 90rpx;margin-left: 30rpx;"></view>
                  <view class="right1 flex-row justify-between" style="margin-left: 80rpx">
                    <image class="right3" src="@/pages-my/static/my4.png" mode="aspectFit"/>
                    <view class="right4 flex-col">
                      <view class="right5 flex-col justify-between">
                        <text class="right6">分享</text>
                        <text class="right7">{{total2}}</text>
                      </view>
                    </view>
                  </view>
              </view>
            ...
            <block v-for="(item,index) in showData" :key="index">
                <view class="box3 flex-row">
                  ...
                  <view class="bd1 flex-col justify-between">
                    ...
                    <text class="txt2">
                      {{item.createTime}}
                      <text v-if="[1,2,5].indexOf(item.type)>-1" style="margin-left: 16rpx;font-family: PingFang-SC-Medium;">树苗活跃度：{{item.refAmount}}</text>
                      <text v-else-if="[6,11,12].indexOf(item.type)>-1" style="margin-left: 16rpx;font-family: PingFang-SC-Medium;">烧伤活跃度：{{item.refAmount}}</text>
                      <text v-else style="margin-left: 16rpx;font-family: PingFang-SC-Medium;">分享活跃度：{{item.refAmount}}</text>                      
                    </text>
                  </view>
...
export default {
  ...
  onLoad() {
      this.global.getFarmerInfo().then((res)=>{
          this.total = res.minerActivityness
          this.total2 = res.promotionActivityness
      }).catch((err)=>{
          this.messageToggle('error','获取果农信息出错：'+err)
      })
      
      let api = '/mining/profile/activityness/query'
      ...
      this.global.D3MetaVerseRequest(api,requestdata,method,true).then(res=>{
          console.log('活跃度流水',res.attachment)
  ...
  onNavigationBarButtonTap(e) {
      this.global.gotoWebView('https://img.tokenai.net/introduction_zy/activity_des.html')
  },
};
</script>
<style>
