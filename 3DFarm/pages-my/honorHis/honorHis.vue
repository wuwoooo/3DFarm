                ...
                <view class="right1 flex-row">
                    <view class="right2 flex-col">
                      <view class="right1 flex-row justify-between">
                        <image class="right3" src="@/pages-my/static/my6.png" mode="aspectFit"/>
                        <view class="right4 flex-col">
                          <view class="right5 flex-col justify-between">
                            <text class="right6">荣誉值</text>
            ...
            <block v-for="item in showData" >
                <view class="box3 flex-row">
                  ...
                  <view class="bd1 flex-col justify-between">
                    ...
                    <text class="txt2">
                      {{item.createTime}} 荣誉值：{{item.postAmount}}
                    </text>
                  </view>
...
export default {
  ...
  onLoad() {
      this.global.getFarmerInfo().then((res)=>{
          this.total = res.reputation
      }).catch((err)=>{
          this.messageToggle('error','获取果农信息出错：'+err)
      })
      
      let api = '/mining/profile/reputation/query'
  ...
  onNavigationBarButtonTap(e) {
        this.global.gotoWebView('https://img.tokenai.net/introduction_zy/honor_des.html')
  },
};
</script>
<style>
